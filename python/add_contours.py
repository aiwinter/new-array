...
def process(filename, key):
    ...
    init_closing = cv2.morphologyEx(
        init_thresh, cv2.MORPH_CLOSE, init_kernel, iterations=500)
    contours = cv2.findContours(
        init_closing.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[1]
    ...
    thresh_2 = cv2.adaptiveThreshold(
